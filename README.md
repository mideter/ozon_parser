...
- Несколько ссылок в интерфейсе: по одной URL в строке; приложение передаёт их **одним** запуском `ozon_fetch.py` — **одно** окно Chromium: страница за страницей (`driver.get` на следующий URL), в конце сессии браузер закрывается; товары объединяются с дедупликацией по URL
